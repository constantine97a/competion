...
import wubi
import jieba
from process_data import generate_traindata
'''
1，生成一个一个的batch
2，将每个sentence转化为id
3，加上unk，start，end
...
def pad_sentence_batch(sentences):
    ...
    for sentence in sentences:
        newsentences.append(list(sentence) + [pad_int]
...
def shuffle_aligned_list(data1, data2, data3):
    ...
    return ([data1[i] for i in p], [data2[i] for i in p], [data3[i] for i in p])  #改成生成器
...
def generate_batch(train_dict, fake_list, batch_size, shuffle=True, is_train=True):
    encoder_inputs, decoder_inputs, simi = generate_traindata(train_dict, fake_list)

    if shuffle:
        encoder_inputs, decoder_inputs, simi = shuffle_aligned_list(
            encoder_inputs, decoder_inputs, simi)
    batch_count = 0
    while True:
        ...
        if is_train:
            ...
            yield (first_batch_pad, second_batch_pad,simi_batch,first_seq_length, second_seq_length)
...
def evaluate(line, batch_size, cl_len = 307):
    lines = listbatch(line, cl_len)
    first_pad , first_seq_length = pad_sentence_batch(lines)
    return (first_pad, first_seq_length)
